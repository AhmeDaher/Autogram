...
function switchLanguage(lang) {
    if (lang === 'ar') {
        ...
        window.location.href = 'en.html?v=' + Date.now();
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Script loaded - Mobile navigation control active');
    ...
    function handleMobileScroll() {
        if (window.innerWidth <= 768) {
            ...
            console.log('Mobile scroll detected:', scrollTop, 'lastScrollTop:', lastScrollTop);
    ...
    // Add search functionality - only for desktop versions (both Arabic and English)
    const isArabicVersion = document.documentElement.lang === 'ar';
    ...
    if (isDesktop) {
        ...
        searchInput.placeholder = isArabicVersion ? 'البحث في الدليل...' : 'Search in guide...';
        searchInput.style.cssText = `
            padding: 10px 15px;
            border: 2px solid #667eea;
            border-radius: 25px;
            outline: none;
            font-family: ${isArabicVersion ? "'Cairo', sans-serif" : "'Inter', sans-serif"};
            direction: ${isArabicVersion ? 'rtl' : 'ltr'};
            width: 200px;
            transition: all 0.3s ease;
        `;
        ...
        // Handle window resize to show/hide search for both versions
        window.addEventListener('resize', function() {
            const currentIsDesktop = window.innerWidth > 768;
            const searchPlaceholder = isArabicVersion ? 'البحث في الدليل...' : 'Search in guide...';
            const existingSearch = document.querySelector(`input[placeholder="${searchPlaceholder}"]`);

            if (currentIsDesktop && !existingSearch) {
                // Re-add search if switched to desktop
                document.body.appendChild(searchContainer);
            } else if (!currentIsDesktop && existingSearch) {
                // Remove search if switched to mobile

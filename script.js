...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add search functionality - only for Arabic version or desktop English version
    const isArabicVersion = document.documentElement.lang === 'ar';
    const isDesktop = window.innerWidth > 768;

    if (isArabicVersion || isDesktop) {
        ...
        // Handle window resize to show/hide search on English version
        if (!isArabicVersion) {
            window.addEventListener('resize', function() {
                const currentIsDesktop = window.innerWidth > 768;
                if (currentIsDesktop && !document.querySelector('input[placeholder="Search in guide..."]')) {
                    // Re-add search if switched to desktop
                    document.body.appendChild(searchContainer);
                } else if (!currentIsDesktop && document.querySelector('input[placeholder="Search in guide..."]')) {
